// ══════════════════════════════════════════════════════
// NŒUD 3 — Préparer Prompt (VERSION FINALE — double type)
//
// Supporte deux modes selon wb.type (envoyé par WordPress) :
//   • 'water_analysis' → analyse structurée, prompt expert court,
//                        données analyse complètes obligatoires
//   • 'chat_message'   → conversation libre avec toutes les options
//
// Champs transmis en sortie :
//   request_type, analyse_id, source → propagés jusqu'au callback
// ══════════════════════════════════════════════════════

const wb = $("Webhook").first().json.body || {};

// ── Type de requête ─────────────────────────────────────
const requestType     = (wb.type || wb.request_type || 'chat_message').toString().trim();
const isWaterAnalysis = requestType === 'water_analysis';

// ── Données piscine ─────────────────────────────────────
const pool    = wb.pool    || {};
const analyse = wb.analyse || {};   // mesures pH/chlore/TAC/stabilisant/température

// ── Options data (chat_message uniquement) ──────────────
...
// ── Alertes eau ─────────────────────────────────────────
// Générées dans les deux modes (utiles au callback WordPress)
const alertes = [];
if (optAlertes || isWaterAnalysis) {
...
// ── Historique de conversation (chat_message seulement) ─
let conversationHistory = [];
if (!isWaterAnalysis && optHistorique && wb.history) {
  try {
    const h = Array.isArray(wb.history) ? wb.history : JSON.parse(wb.history);
    if (Array.isArray(h)) conversationHistory = h.slice(-20);
  } catch (e) { conversationHistory = []; }
}

// ══════════════════════════════════════════════════════
// DOCUMENTATION PISCINE INTÉGRÉE
// ══════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════
// PRODUITS (chat_message uniquement)
// ══════════════════════════════════════════════════════
const CATS_LABELS = {
...
const produitsBruts = wb.produits || pool.produits || null;

if (!isWaterAnalysis && produitsBruts && produitsBruts.length > 0) {
  if (typeof produitsBruts[0] === 'string') {
    ...
    produitsTexte = produitsBruts.map((p, idx) => {
      ...
      return `  Produit ${idx + 1}: ${nomComplet}${qte ? ' — stock: ' + qte : ''}${dateTag}${commentTag}${photosTag}`;
    }).join('\n');
  }
} else if (!isWaterAnalysis) {
...
// ── Résumé des mesures pour les deux prompts ───────────
const analyseResume = (() => {
  const vals = [];
  if (analyse.ph          !== null && analyse.ph          !== undefined) vals.push(`pH ${analyse.ph}`);
  if (analyse.chlore      !== null && analyse.chlore      !== undefined) vals.push(`Chlore ${analyse.chlore} mg/L`);
  if (analyse.tac         !== null && analyse.tac         !== undefined) vals.push(`TAC ${analyse.tac} mg/L`);
  if (analyse.stabilisant !== null && analyse.stabilisant !== undefined) vals.push(`Stabilisant ${analyse.stabilisant} mg/L`);
  if (analyse.temperature !== null && analyse.temperature !== undefined) vals.push(`Temp ${analyse.temperature}°C`);
  return vals.length > 0 ? vals.join(' | ') : 'aucune mesure fournie';
})();
...
const systemPromptChat = `Tu es Sunny 🌞 — un assistant virtuel avec la personnalité d'un technicien piscine sympa, expérimenté, et humain.
...
const systemPrompt = isWaterAnalysis ? systemPromptAnalyse : systemPromptChat;
...
// Image principale (eau, bandelette, piscine)
if (wb.image_base64) {
  const imageUrl = cleanBase64(wb.image_base64);
  if (imageUrl) imageItems.push({ type: 'image_url', image_url: { url: imageUrl, detail: 'high' } });
}

// Photos produits (chat_message uniquement)
if (!isWaterAnalysis && optProduits && Array.isArray(wb.produits)) {
...
// ── Message utilisateur ─────────────────────────────────
const defaultMsg = isWaterAnalysis
  ? "Analyse ces mesures d'eau et propose un plan d'action concret pour rééquilibrer le bassin."
...
// ══════════════════════════════════════════════════════
// RETURN — tous les champs propagés jusqu'au callback
// ══════════════════════════════════════════════════════
return [{
  json: {
    // ── Routing & identification ──────────────────────
    request_type:     requestType,                         // 'water_analysis' | 'chat_message'
    analyse_id:       wb.analyse_id      || '',            // ID DB si water_analysis
    source:           wb.source          || '',
    ...
    // ── Données enrichies pour le callback ────────────
    alertes,
    planning:         isWaterAnalysis ? {} : planning,
